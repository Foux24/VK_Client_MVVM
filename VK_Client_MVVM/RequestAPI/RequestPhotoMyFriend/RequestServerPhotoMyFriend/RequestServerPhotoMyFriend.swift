//
//  RequestServerPhotoMyFriend.swift
//  VK_Client_MVVM
//
...
final class RequestServerPhotoMyFriend {
    ...
    /// Метод загрузки фото друзей
    func loadPhoto(idFriend: String, completion: @escaping (Result<[PhotoFriend], PhotoFriendsError>) -> Void) {
        DispatchQueue.global(qos: .utility).async { [self] in
            ...
            let task = session.dataTask(with: url) { data, response, error in
                ...
                do {
                    let result = try self.decoder.decode(PhotosFriend.self, from: data)
...
// MARK: - Private
private extension RequestServerPhotoMyFriend {
